...
| `hal task edit --notes '<notes>' --message '<reason>'` | タスクのノート更新 |
...
中間結果や重要な発見は `hal task edit --notes '<content>' --message '<reason>'` で記録してください。
notes はセッションをまたいで引き継がれます。
...
- 作業中にユーザーから再利用可能なフィードバックを受けた場合:
  ...
  2. ファイルに内容を書き込む
  3. `hal context log --id <id> --type updated --message '<msg>'` で記録
