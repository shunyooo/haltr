...
await testAsync("hal init creates haltr/ directory", async () => {
	...
	try {
		...
		assert(
			existsSync(join(dir, "haltr", "config.yaml")),
...
await testAsync("hal start creates tmux session", async () => {
	...
	try {
		...
		assert(
			panes.length >= 1,
			`should have at least 1 pane, got ${panes.length}`,
		);
	} finally {
		cleanup();
		rmSync(dir, { recursive: true, force: true });
...
await testAsync("full flow: init → epic → task → session → panes", async () => {
	...
	try {
		...
		writeFileSync(
			taskPath,
			yaml.dump(taskWithSteps, { lineWidth: -1 }),
			"utf-8",
		);
		...
		assert(
			panesOut.includes("No panes") || panesOut.includes("PANE"),
			`panes output: ${panesOut}`,
		);

		// Epic list
		const listOut = halInDir(dir, "epic", "list");
		assert(
			listOut.includes("test-feature"),
			`epic list should include test-feature: ${listOut}`,
		);
...
await testAsync("hal spawn creates new tmux pane", async () => {
	...
	try {
		...
		const panesOut = halInDir(dir, "panes");
		assert(
			panesOut.includes("worker"),
			`panes should show worker: ${panesOut}`,
		);
		assert(
			panesOut.includes("step-1"),
			`panes should show step-1: ${panesOut}`,
		);
	} finally {
		cleanup();
		rmSync(dir, { recursive: true, force: true });
...
await testAsync("hal spawn verifier creates separate pane", async () => {
	...
	try {
		...
		assert(
			panes.length >= 3,
			`should have at least 3 panes (base + worker + verifier): got ${panes.length}`,
		);
...
await testAsync("hal kill removes task panes from tmux", async () => {
	...
	try {
		...
		assert(
			beforePanes.length >= 2,
			`should have >= 2 panes before kill: ${beforePanes.length}`,
		);
		...
		assert(
			!panesOut.includes("worker"),
			`worker should be gone from tracking: ${panesOut}`,
		);
	} finally {
		cleanup();
		rmSync(dir, { recursive: true, force: true });
...
await testAsync("full workflow: start → history → status → check", async () => {
	...
	try {
		...
		assert(
			...
			`should have >= 3 history events: ${updated.history.length}`,
		);
...
await testAsync(
	"renderHooks generates settings.yaml and settings.json for Claude",
	async () => {
		const dir = createTestDir();
		try {
			halInDir(dir, "init");
			...
			const epics = readdirSync(epicDir).filter(
				(e: string) => !e.startsWith(".") && e !== "archive",
			);
			...
			assert(
				workerDir !== undefined,
				`worker hooks dir should exist: ${hooksDirs}`,
			);
			...
			const settingsContent = yaml.load(
				readFileSync(settingsYaml, "utf-8"),
			) as any;
...
await testAsync("hal epic archive moves epic to archive/", async () => {
	...
	try {
		...
		assert(
			archiveOut.includes("Archived") || archiveOut === "",
			`archive: ${archiveOut}`,
		);
		...
		const listOut = halInDir(dir, "epic", "list");
		assert(
			!listOut.includes("done-feature"),
			`should not be in list: ${listOut}`,
		);
