...
test("hal init creates haltr/ with all required files and directories", () => {
  ...
  try {
    ...
    assert(existsSync(join(haltrDir, "config.yaml")), "config.yaml not created");
    ...
    assert(config.retry.max_attempts === 3, "config.retry.max_attempts should be 3");
...
test("hal init when haltr/ already exists -> error", () => {
  ...
  try {
    ...
    try {
      ...
      assert(msg.includes("already exists"), `Expected 'already exists' in error, got: ${msg}`);
    }
...
test("hal task new creates 001_task.yaml with created event and status: pending", () => {
  ...
  try {
    ...
    assert(taskPath.endsWith("001_task.yaml"), `Expected 001_task.yaml, got: ${taskPath}`);
...
test("Second hal task new -> 002_task.yaml, old gets pivoted event + pivoted status, new has previous field", () => {
  ...
  try {
    ...
    assert(secondPath.endsWith("002_task.yaml"), `Expected 002_task.yaml, got: ${secondPath}`);

    // Check the old task
    const oldContent = readFileSync(firstPath, "utf-8");
    const oldTask = yaml.load(oldContent) as TaskYaml;
    assert(oldTask.status === "pivoted", `Expected old task status 'pivoted', got '${oldTask.status}'`);
    ...
    assert(newTask.status === "pending", `Expected new task status 'pending', got '${newTask.status}'`);
  } finally {
    rmSync(tmp, { recursive: true, force: true });
  }
});
...
test("Gap handling: if 001_task.yaml and 003_report.md exist -> next is 004_task.yaml", () => {
  ...
  try {
    ...
    writeFileSync(join(epicPath, "001_task.yaml"), yaml.dump(task001, { lineWidth: -1 }));
...
test("hal task edit adds updated event to history", () => {
  ...
  try {
    ...
    assert(!isNaN(atDate.getTime()), "Expected 'at' to be a valid ISO 8601 date");
  } finally {
    rmSync(tmp, { recursive: true, force: true });
  }
});
...
test("Multiple edits accumulate history events", () => {
  ...
  try {
    ...
    assert(task.history!.length === 3, `Expected 3 history events (created + 2 updated), got ${task.history!.length}`);
    assert(task.history![0].type === "created", "First event should be created");
    assert(task.history![1].type === "updated", "Second event should be updated");
    assert(task.history![2].type === "updated", "Third event should be updated");
...
test("Three tasks in sequence with correct pivoting chain", () => {
  ...
  try {
    ...
    assert(task3.endsWith("003_task.yaml"), `Expected 003_task.yaml, got: ${task3}`);
    ...
    assert(t2Pivoted?.next_task === "003_task.yaml", "Task 2 pivot should point to task 3");
