...
test("hal --help shows subcommands", () => {
	const output = execSync("node dist/bin/hal.js --help", {
...
test("hal --version shows version", () => {
	const output = execSync("node dist/bin/hal.js --version", {
		cwd: "/workspaces/haltr",
		encoding: "utf-8",
	});
...
const fullTaskData = {
			...
			steps: [
				{
					id: "data-collection",
					instructions:
						"Collect reference x generated image pairs from prd logs",
	...
	history: [
		...
		},
		{
			...
			attempt: 1,
			message:
				"Implemented fetch_pairs.py. Fetched 2 weeks of data from BQ and generated 152 JSON files in data/pairs/",
		},
		{
...
test("Accept as string shorthand", () => {
	...
	if (
		acceptArr[0].id !== "default" ||
		acceptArr[0].check !== "npm test passes"
	) {
...
test("Step status 'pivoted' -> error (invalid for steps)", () => {
	const data = {
		...
		agents: { worker: "claude", verifier: "codex" },
		steps: [{ id: "s1", instructions: "test", status: "pivoted" }],
	};
	expectThrows(() => validateTask(structuredClone(data)));
});
...
test("Step status 'skipped' -> PASS (valid for steps)", () => {
	const data = {
		...
		agents: { worker: "claude", verifier: "codex" },
		steps: [{ id: "s1", instructions: "test", status: "skipped" }],
	};
	validateTask(structuredClone(data));
});
...
test("Valid config.yaml validates successfully", () => {
	...
	if (result.orchestrator_cli !== "claude") {
		throw new Error(
			`Expected orchestrator_cli "claude", got "${result.orchestrator_cli}"`,
		);
	}
});
...
test("loadAndValidateConfig with YAML file", () => {
	...
	if (result.retry.max_attempts !== 3) {
		throw new Error(
			`Expected max_attempts 3, got ${result.retry.max_attempts}`,
		);
	}
});
...
test("Config: retry.max_attempts must be positive integer", () => {
	...
	(data.retry as Record<string, unknown>).max_attempts = 0;
	expectThrows(() => validateConfig(data));
});

test("Config: retry.max_attempts must be integer (not float)", () => {
	const data = structuredClone(validConfig);
	(data.retry as Record<string, unknown>).max_attempts = 1.5;
	expectThrows(() => validateConfig(data));
});
...
test("History: all event types in full task sample", () => {
	...
	if (!types.includes("step_started"))
		throw new Error("Missing 'step_started' event");
	if (!types.includes("work_done"))
		throw new Error("Missing 'work_done' event");
	if (!types.includes("verifier_started"))
		throw new Error("Missing 'verifier_started' event");
	if (!types.includes("verification_passed"))
		throw new Error("Missing 'verification_passed' event");
	if (!types.includes("verification_failed"))
		throw new Error("Missing 'verification_failed' event");
});
