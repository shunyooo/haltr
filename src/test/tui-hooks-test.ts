...
await asyncTest(
	"fs.watch detects task.yaml changes and loadTasks returns updated data",
	async () => {
		...
		if (!changed) {
			throw new Error(
				"fs.watch did not detect task.yaml change within timeout",
			);
		}
...
await asyncTest(
	"fs.watch detects .panes.yaml changes and loadPanes returns updated data",
	async () => {
		...
		if (!changed) {
			throw new Error(
				"fs.watch did not detect .panes.yaml change within timeout",
			);
		}
