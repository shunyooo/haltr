...
function readTask(path: string): TaskYaml {
...
if (!existsSync(HAL_BIN)) {
	console.log("ERROR: run 'npm run build' first");
	process.exit(1);
}
...
`,
	...
	await sleep(500);
	const workerPaneId = tmux("list-panes", "-t", SESSION, "-F", "#{pane_id}")
		.split("\n")
		.pop()!;
...
`,
	...
	if (!existsSync(targetFile))
		throw new Error("hello.txt not created by worker");
	...
	const workDone = task2.history?.find((e) => e.type === "work_done");
	if (!workDone) {
	...
	tmux("split-window", "-t", SESSION, "-c", testDir);
	await sleep(500);
	const verifierPaneId = tmux("list-panes", "-t", SESSION, "-F", "#{pane_id}")
		.split("\n")
		.pop()!;
...
`,
	...
	const verPassed = task3.history?.find(
		(e) => e.type === "verification_passed",
	);
	const verFailed = task3.history?.find(
		(e) => e.type === "verification_failed",
	);

	if (verPassed) {
		console.log("   ✓ verification_passed recorded");
		const msg = "message" in verPassed ? verPassed.message : "";
		console.log(`     Evidence: ${msg}`);
	} else if (verFailed) {
		const msg = "message" in verFailed ? verFailed.message : "";
		console.log(`   △ verification_failed: ${msg}`);
	} else {
	...
	for (const event of finalTask.history ?? []) {
		...
		const extra = "message" in event ? event.message ?? "" : "";
		console.log(
			`     ${time} ${event.type}${step} ${extra ? "— " + extra : ""}`,
		);
	}
	...
	if (verPassed && finalTask.steps?.[0]?.status !== "done")
		errors.push(`Step not done: ${finalTask.steps?.[0]?.status}`);

	const expectedEvents = ["created", "step_started"];
	for (const type of expectedEvents) {
		if (!finalTask.history?.some((e) => e.type === type)) {
