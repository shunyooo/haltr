...
import { formatPanesTable, handlePanes } from "../commands/panes.js";
...
await testAsync(
	...
	async () => {
		...
		try {
			...
			assertEqual((escEvent as any).message, "API unavailable", "reason");
			assertEqual((escEvent as any).step, "step-1/data-collection", "step");
			assert((escEvent as any).attempt >= 1, "attempt should be >= 1");
			assert(
...
await testAsync("escalate: notifies parent via tmux send-keys", async () => {
	const dir = createTestDir();
	try {
		const taskPath = createTaskFile(dir, makeBaseTask());
		const mock = createMockSendKeys();
...
await testAsync(
	...
	async () => {
		...
		try {
			...
			assert(/^\d{4}-\d{2}-\d{2}T/.test(escEvent.at), "at should be ISO 8601");
...
await testAsync("escalate: step not found -> error", async () => {
	const dir = createTestDir();
	try {
		const taskPath = createTaskFile(dir, makeBaseTask());
		const mock = createMockSendKeys();
...
await testAsync(
	...
	async () => {
		...
		try {
			...
			writeFileSync(
				otherTaskPath,
				yaml.dump(makeBaseTask(), { lineWidth: -1, noRefs: true }),
			);
			...
			await handleKill({ task: taskPath }, mockKill.fn, dir);
...
await testAsync("kill: already-dead pane doesn't cause error", async () => {
	const dir = createTestDir();
	try {
		const mockKill = createMockKillPane();
...
await testAsync("kill: no matching panes -> no error", async () => {
	const dir = createTestDir();
	try {
		const mockKill = createMockKillPane();
		const taskPath = createTaskFile(dir, makeBaseTask());
...
test("epic list: lists epics with status (excludes archive)", () => {
	...
	try {
		...
		writeFileSync(
			join(epicsDir, "20260319-001_implement-auth", "001_task.yaml"),
			yaml.dump({
				id: "auth",
				status: "in_progress",
				agents: { worker: "claude", verifier: "codex" },
...
test("epic archive: moves to archive/", () => {
	...
	try {
		...
		assert(
			existsSync(join(epicsDir, "archive", "20260319-001_test-epic")),
			"should exist in archive",
		);

		// Content should be preserved
		const content = readFileSync(
			join(epicsDir, "archive", "20260319-001_test-epic", "001_task.yaml"),
			"utf-8",
		);
...
test("epic archive: error if destination exists", () => {
	...
	try {
		...
		mkdirSync(join(epicsDir, "archive", "20260319-001_dupe-epic"), {
			recursive: true,
		});
...
test("epic archive: not found -> error", () => {
	const dir = createTestDir();
	try {
		expectThrows(() => archiveEpic(dir, "nonexistent"), "No epic found");
	} finally {
		rmSync(dir, { recursive: true, force: true });
	}
});
...
test("rule add: appends rule to rules.md", () => {
	...
	try {
		...
		assert(content.includes("- Always write tests"), "should contain the rule");
	} finally {
		rmSync(dir, { recursive: true, force: true });
	}
});
