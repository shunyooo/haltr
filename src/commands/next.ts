...
export async function handleNext(
	...
	if (toStep.status && toStep.status !== "pending") {
		throw new Error(
			`Step "${opts.to}" is not pending (current: ${toStep.status})`,
		);
	}
	...
	task.history.push(stepStartedEvent);
	...
	if (isShared) {
		...
		if (workerPane) {
			...
			try {
				...
				console.log(
					`Worker pane ${workerPane.pane_id} に次のステップを送信しました`,
				);
			} catch {
				// Pane might be dead — fall back to spawn
				console.log(
					"Worker pane が見つかりません。新しい worker を spawn します。",
				);
				await handleSpawn(
	...
	if (step.accept) {
		if (typeof step.accept === "string") {
			...
			for (const a of step.accept) {
				...
				msg += `  - ${acceptObj.id}: ${acceptObj.check}\n`;
			}
		}
	}
...
function countAttempts(history: HistoryEvent[], stepId: string): number {
