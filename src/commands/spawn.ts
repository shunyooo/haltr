...
import { tmuxEnableBorderStatus, tmuxStylePane } from "../lib/tmux.js";
import { loadAndValidateTask } from "../lib/validator.js";
import type { AcceptObject, ConfigYaml, TaskYaml } from "../types.js";
...
export function resolveCli(
	...
	const isOrchestrator =
		role === "sub-orchestrator" || role === "task-spec-reviewer";
	...
	// 4. Task-level
	if (isWorker) {
		return taskYaml.agents?.worker ?? configYaml.defaults?.worker ?? configYaml.orchestrator_cli;
	}
	if (isVerifier) {
		return taskYaml.agents?.verifier ?? configYaml.defaults?.verifier ?? configYaml.orchestrator_cli;
	}
...
export function convertAgentSettingsForClaude(
	hooksDir: string,
): ClaudeSettings {
	...
	if (parsed.hooks && typeof parsed.hooks === "object") {
		...
		for (const [event, entries] of Object.entries(haltrHooks)) {
			...
			claudeHooks[event] = entries.map((entry: unknown) => {
				const e = entry as { matcher?: string; command?: string };
				return {
					matcher: e.matcher ?? "",
					hooks: e.command ? [{ type: "command", command: e.command }] : [],
				};
			});
		}
...
export function buildLaunchScript(
	...
	if (provider !== "claude") {
		...
		if (agentSettings.settingsPath)
			parts.push(`--settings '${agentSettings.settingsPath}'`);
		if (agentSettings.allowedTools?.length)
			parts.push(`--allowedTools "${agentSettings.allowedTools.join(",")}"`);
		if (agentSettings.disallowedTools?.length)
			parts.push(
				`--disallowedTools "${agentSettings.disallowedTools.join(",")}"`,
			);
		if (agentSettings.permissionMode)
			parts.push(`--permission-mode ${agentSettings.permissionMode}`);
		...
		const isOrchestrator =
			role === "main-orchestrator" || role === "sub-orchestrator";
		if (!isOrchestrator) {
			const userMsg =
 ...
 */
function buildStepDetails(taskYaml: TaskYaml, stepPath: string): string {
	const step = findStep(taskYaml.steps, stepPath);
	if (!step) {
 ...
 */
function buildAcceptCheckDetails(taskYaml: TaskYaml, stepPath: string): string {
	const step = findStep(taskYaml.steps, stepPath);
	if (!step) {
		return `Step: ${stepPath} (not found)`;
	}
...
export function assemblePrompt(
	...
	const acceptDetails = stepPath
		? buildAcceptCheckDetails(taskYaml, stepPath)
		: "";
	let stepsOverview = "";
	for (const step of taskYaml.steps) {
		...
		if (step.accept) {
			if (typeof step.accept === "string") {
				...
				for (const a of step.accept) {
					const acceptObj = a as AcceptObject;
					stepsOverview += `  accept [${acceptObj.id}]: ${acceptObj.check}\n`;
				}
			}
		}
	}
