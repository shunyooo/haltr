...
import { tmuxKillPane, tmuxSendKeys } from "../lib/tmux.js";
import { loadAndValidateTask } from "../lib/validator.js";
import type { AcceptObject, HistoryEvent, Step, TaskYaml } from "../types.js";
...
/**
 * Type guard: check if event has step and attempt fields.
 */
function hasStepAndAttempt(
	e: HistoryEvent,
): e is Extract<HistoryEvent, { step: string; attempt: number }> {
	return "step" in e && "attempt" in e;
}
...
function getStepHistory(
	...
	return history.filter((e) => {
		if (!hasStepAndAttempt(e) || e.step !== stepId) return false;
		if (e.attempt !== currentAttempt) return false;
		return true;
	});
}
 ...
 */
export function checkWorker(task: TaskYaml, stepId: string): CheckResult {
	...
	if (task.worker_session === "shared") {
		...
		if (hasPendingSteps) {
			return {
				action: "block",
				message:
					"セッション共有モードです。次のステップの指示を待ってください。",
			};
		}
	}
	...
	if (hasVerificationFailedAfterWorkDone) {
		return {
			action: "block",
			message: `検証が失敗しています。修正を完了してから以下を実行してください: hal history add --type work_done --step '${stepId}' --task <task-path> --message '修正内容'`,
		};
	}
 ...
 */
export function checkVerifier(task: TaskYaml, stepId: string): CheckResult {
	const history = task.history ?? [];
	const stepHistory = getStepHistory(history, stepId);
...
async function cleanupPane(
	...
	try {
		...
		if (myPane) {
			...
			try {
				await tmuxKillPane(myPane.pane_id);
			} catch {}
		}
	} catch {
		// Best effort
...
async function handleCheck(opts: {
	...
	if (modeCount > 1) {
		throw new Error("Only one mode can be specified");
	}
	...
	if (opts.orchestrator) {
		...
		if (result.action === "block") {
			...
			console.log(
				JSON.stringify({ systemMessage: `[haltr] orchestrator check passed` }),
			);
			process.exit(0);
		}
	}
	...
	if (opts.taskSpecReviewer) {
		...
		if (parentPaneId) {
			try {
				await tmuxSendKeys(
					parentPaneId,
					`spec_reviewed: タスク仕様のレビューが完了しました。task.yaml の history を確認してください。`,
				);
			} catch {
				// Best effort
			}
		...
		if (reviewerPane) {
			...
			try {
				await tmuxKillPane(reviewerPane.pane_id);
			} catch {}
		}

		console.log(
			JSON.stringify({ systemMessage: "[haltr] spec review completed" }),
		);
		process.exit(0);
	}
}
