 ...
 */
const STEP_NOT_ALLOWED_TYPES = new Set([
	"completed",
	"spec_reviewed",
	"execution_approved",
	"user_feedback",
...
function handleHistoryAdd(opts: {
	...
	const { type, task: taskPath, step, message, acceptId } = opts;
	...
	if (
		...
		if (foundStepObj?.accept && Array.isArray(foundStepObj.accept)) {
			const acceptObj = (
				foundStepObj.accept as import("../types.js").AcceptObject[]
			).find((a) => a.id === acceptId);
			if (acceptObj?.type) {
